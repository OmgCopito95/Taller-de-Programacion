...
begin
	...
	for i := 1 to cantE do 
		if (todos[i] <> NIL) and (todos[i]^.dato.impuesto <= impuestoMin.impuesto) then
		begin
			posMin := i;
			impuestoMin.impuesto := todos[i]^.dato.impuesto;
		end;

	if (posMin <> -1) then
	begin
		impuestoMin := todos[posMin]^.dato;
...
begin
	...
	minimo(todos,impMin);
	while (impMin.impuesto <> marca_fin) do
	begin
		impActual.monto := 0;
		impActual.impuesto := impMin.impuesto;
		
		while (impMin.impuesto <> marca_fin) and (impMin.impuesto = impActual.impuesto) do begin
			impActual.monto := impActual.monto + impMin.monto;
			minimo(todos,impMin);
		end;
		
		agregarAtras(L,ult,impActual);
